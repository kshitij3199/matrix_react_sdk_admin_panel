...
export default React.createClass({
    ...
    render: function() {
        ...
        if (this.state.view === VIEWS.ADMIN) {
            // store errors stop the client syncing and require user intervention, so we'll
            // be showing a dialog. Don't show anything else.
            const isStoreError = this.state.syncError && this.state.syncError instanceof Matrix.InvalidStoreError;
            ...
            if (this.state.ready && this.state.page_type && !isStoreError) {
                 ...
                 * as using something like redux to avoid having a billion bits of state kicking around.
                 */
                const LoggedInView = sdk.getComponent('structures.AdminLoggedInView');
                return (
                   <LoggedInView ref={this._collectLoggedInView} matrixClient={MatrixClientPeg.get()}
                        onRoomCreated={this.onRoomCreated}
                        onCloseAllSettings={this.onCloseAllSettings}
        ...
        // if (this.state.view === VIEWS.ADMIN) {
        //     const Admin = sdk.getComponent('structures.auth.Admin');
            
        //     return (<Admin ref={this._collectLoggedInView} matrixClient={MatrixClientPeg.get()}
        //     onRoomCreated={this.onRoomCreated}
        //     onCloseAllSettings={this.onCloseAllSettings}
        //     onRegistered={this.onRegistered}
        //     currentRoomId={this.state.currentRoomId}
